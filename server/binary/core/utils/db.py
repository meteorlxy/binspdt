  ...
  def get_module_details(self, module_id):
    ...
    try:
      with conn:
        with conn.cursor() as curs:
          curs.execute('SELECT \
            (SELECT COUNT(*) FROM ex_%(module_id)s_functions WHERE type=0) AS module_functions_count, \
            (SELECT COUNT(*) FROM ex_%(module_id)s_functions WHERE type!=0) AS external_functions_count, \
      ...
      with conn:
        with conn.cursor() as curs:
          curs.execute('SELECT func.*, \
            ...
            LEFT JOIN ex_%(module_id)s_basic_blocks AS bb ON bb.parent_function=func.address', {
            'module_id': module_id,
          })
          return zip_result(curs)
    finally:
      ...
      with conn:
        with conn.cursor() as curs:
          curs.execute('SELECT func.*, \
            ...
            FROM ex_%(module_id)s_functions AS func \
            LEFT JOIN ex_%(module_id)s_basic_blocks AS bb ON bb.parent_function=func.address \
            LEFT JOIN ex_%(module_id)s_basic_block_instructions AS bb_inst ON bb.id=bb_inst.basic_block_id \
            LEFT JOIN ex_%(module_id)s_instructions AS inst ON bb_inst.instruction=inst.address', {
            'module_id': module_id,
          })
          return zip_result(curs)
    finally:
      ...
      with conn:
        with conn.cursor() as curs:
          curs.execute('SELECT func.*, \
            ...
            LEFT JOIN ex_%(module_id)s_basic_block_instructions AS bb_inst ON bb.id=bb_inst.basic_block_id \
            LEFT JOIN ex_%(module_id)s_instructions AS inst ON bb_inst.instruction=inst.address \
            LEFT JOIN ex_%(module_id)s_operands AS op ON inst.address=op.address', {
            'module_id': module_id,
          })
          return zip_result(curs)
    finally:
